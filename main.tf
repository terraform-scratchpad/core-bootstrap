...
resource "azurerm_resource_group" "core-state-rg" {
...
resource "azurerm_storage_account" "core-state-storage-account" {
  account_replication_type  = "LRS"
  account_tier              = "Standard"
  location                  = "${var.location}"
  name                      = "${var.tfstate_storage_account_name}"
  resource_group_name       = "${azurerm_resource_group.core-state-rg.name}"
  tags                      = "${var.tags}"
}


resource "azurerm_storage_container" "core-state-container" {
  name                      = "${var.tfstate_storage_container_name}"
  resource_group_name       = "${azurerm_resource_group.core-state-rg.name}"
  storage_account_name      = "${azurerm_storage_account.core-state-storage-account.name}"
}
